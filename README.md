...
### Features - Backend Functionality, Site Security & Form Validation

| **Page / Feature** | **Visible To** | **Template Rendered** | **Back End Functionality** | **Front End Form Validation** | **Back End Form Validation** | **Front End Security** | **Back End Security** | **Routing - log in** | **Routing - other** |
|---|---|---|---|---|---|---|---|---|---|
| **Home** | All Users | index.html | None | N/a | N/a | Contents of info text div change based on whether user is logged in | None | None | None |
| **Join** | All Users | join.html | Creates new user in db. Adds user to session cookie (log in) | All fields are required. Fields must match correct type and length using built in HTML validation. | Username must not already exist and passwords must match | None | None | Redirects to 'events' on log-in | None |
| **Sign In** | All Users | sign-in.html | Adds user to session cookie (log in) | All fields are required. Fields must match correct type and length using built in HTML validation. | Password & username must exist & be correct | None | None | User redirect here if page for logged-in users only. Returns user to original page they attempted to visit, or to 'profile' | None |
| **Sign Out** | Logged In Users | None | Removes user & any stored url from session cookie | N/a | N/a | None | User must be logged in | Redirects to 'sign-in' on log-out | None |
| **Profile** | Logged In Users | my-profile.html | Populates page with current user's details and any events created by user including event & location details from database. Events filtered by future only & sorted by date. | N/a | N/a | Edit / Delete Buttons only visible on user's own events or all events if admin - additional layer of security. | User must be logged in | If user not logged in re-routes to 'sign-in' (session url not stored for logged-out users as this is default redirect after sign in). | Session url stored to redirect back after editing or deleting event via this page. |
| **Events** | Logged In Users | events.html | Populates page with events including event & location details from database. Events filtered by future only & sorted by date. | N/a | N/a | Edit / Delete Buttons only visible on user's own events or all events if admin. | User must be logged in | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. | Session url stored to redirect back here after editing or deleting event via this page. |
| **Event** | Logged In Users | event.html | Populates page with event and location information | N/a | N/a | Edit / Delete Buttons only visible if event created by current user or user is admin | User must be logged in | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. | Session url stored to redirect back here after editing event via this page. |
| **Add Event** | Logged In Users | add-event.html | Populates form with locations, whos & challenge-levels for user selection. Submission: converts date & time to UTC format, adds location id to event, adds event to database. | All fields required & must match type & length (HTML validation). Location, Who & Challenge - dropdown lists (not directly editable). Date & Time populated using pickers (not directly editable). Event must not be in the past (validated by form.js) | None | None | User must be logged in | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. | None |
| **Edit Event** | Logged In Users - own events only. Admin - all events. | edit-event.html | Populates form with event data, locations, whos & challenge-levels for user selection / edit. Submission: converts date & time to UTC format, adds location id to event, updates event on database. | All fields required & must match type & length (HTML validation). Location, Who & Challenge - dropdown lists (not directly editable). Date & Time populated using pickers (not directly editable). Event must not be in the past (validated by form.js) | None | None | User must be logged in & admin or event must be user's own. | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. Id for session url is taken from page url. | Redirects to previous page after editing unless session url is this page (from login redirect). Defaults to 'events' page. |
| **Delete Event** | Logged In Users - own events only. Admin - all events. | None | Removes event from database. | N/a | N/a | Delete buttons on other pages only visible for admin or on user's own events. | User must be logged in & admin or event must be user's own. | If user not logged in re-routes to 'sign-in' & stores session url for 'events' page to redirect there post log-in. This adds a layer of protection when deleting events. | Redirects to previous page after deleting unless session url is for the deleted 'event' page.  Defaults to 'events' page. |
| **Manage Locations** | Admin Only | manage-locations.html | Populates page with all locations. | N/a | N/a | None | User must be logged in and admin | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. | Session url stored to redirect back here after editing or deleting a location via this page. |
| **Location** | All Users | location.html | Populates page with location information & any events at this location. Events filtered by future only & sorted by date. | N/a | N/a | Location Edit / Delete Buttons only visible for admin. Events only visible for logged in users. Event Edit / Delete Buttons only visible to admin or on user's own events. | None | None | Session url stored to redirect back here after editing a location via this page. |
| **Add Location** | Admin Only | add-location.html | Submission: uploads image to Cloudinary & adds image url to location data. Adds location to database. | All fields required and must match type and length (HTML validation). Latitude & Longitude populated using Map Picker (not directly editable). Image under 5mb (file-validation.js) and correct format (HTML validation). | Checks image is under 5MB & correct formats - redirects to 413/415 page if not. | None | User must be logged in and admin. | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. | None |
| **Edit Location** | Admin Only | edit-location.html | Populates form with location data. Submission: checks if new image uploaded & replaces it if so. Uploads image to Cloudinary & adds image url to location data. Updates location on database. | All fields required except image upload (to keep old image) and must match type and length (HTML validation). Latitude & Longitude populated using Map Picker (not directly editable). Image under 5mb (file-validation.js) and correct format (HTML validation). | Checks image is under 5MB & correct formats - redirects to 413/415 page if not. | None | User must be logged in and admin. | If user not logged in re-routes to 'sign-in' & stores session url to redirect here post log-in. Id for session url is taken from page url. | Redirects to previous page after editing unless session url is this page (from login redirect). Defaults to 'manage-locations' page. |
| **Delete Location** | Admin Only | None | Removes location from database. | N/a | N/a | Delete buttons on other pages only visible for admin. | User must be logged in & admin. | If user not logged in re-routes to 'sign-in' & stores session url for 'manage-locations' page to redirect there post log-in. This adds a layer of protection when deleting locations. | Redirects to manage-locations page. |  |
| **404 Error** | All Users | 404.html | None | N/a | N/a | N/a | N/a | N/a | Redirects here when user enters a non-existent page |
| **413 Error** | All Users | 413.html | None | N/a | N/a | N/a | N/a | N/a | Redirects here if user uploads an image larger than 5MB |
| **415 Error** | All Users | 415.html | None | N/a | N/a | N/a | N/a | N/a | Redirects here if file uploaded in incorrect format |
| **500 Error** | All Users | 500.html | None | N/a | N/a | N/a | N/a | N/a | Redirects here after a server error |
